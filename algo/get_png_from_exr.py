...
LastEditTime: 2026-01-29 15:51:29
...
def hdr_to_rgb(hdr_image,adjust_gamma = False):
    # 对HDR图像进行色调映射
...
if __name__ == '__main__':
    ...
    imgs = glob.glob(os.path.join(root, '**/*.exr'), recursive=True)
    for index in tqdm(range(len(imgs))):
        img =imgs[index]
        hdr_image = read(img)[...,:3]
        cimage = hdr_to_rgb(hdr_image)
        target_image = cv2.resize(cimage,(1920,1080),None)
        sp = img.replace(root,save_path).replace('.exr','.png')
        write(sp,target_image)
