...
def scene_change_detect(images):
    ...
    for seq,data in images.items():
        seq_images = data[0]
        assert len(seq_images)>=2,'the number of images must >=2!'
        all_images = None
        for image_ in seq_images:
            if not os.path.isfile(image_):
                continue
            image = read(image_,type='image')
            image = cv2.resize(image,(48,27))
...
def scene_change_core(all_images,model,DEVICE):
    ...
    for batch_ in batches_images:
        batch = torch.from_numpy(batch_.transpose((3, 0, 1, 2))[np.newaxis, ...]) * 1.0
        batch = batch.to(DEVICE)
        one_hot = model(batch)
        if isinstance(one_hot, tuple):
            one_hot = one_hot[0]
        one_hot = torch.sigmoid(one_hot[0]).detach().cpu().numpy()
        predictions.append(one_hot[25:75])
    predictions = np.concatenate(predictions, 0)[:len(all_images)]
    return predictions
