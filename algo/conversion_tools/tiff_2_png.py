...
LastEditTime: 2024-10-25 16:11:08
...
if __name__ == '__main__':
    ...
    assert len(sys.argv)==3 ,'usage: python tiff_2_png.py root save_path'
    root = sys.argv[1]
    ...
    for index in tqdm(range(len(imgs))):
        img =imgs[index]
        hdr_image = cv2.imread(img)[...,:3]
        cimage = hdr_image
        # cimage = hdr_to_rgb(hdr_image)
        sp = os.path.join(save_path,os.path.basename(img)).replace('.tiff','.png').replace('.tif','.png')
        cv2.imwrite(sp,cimage)
