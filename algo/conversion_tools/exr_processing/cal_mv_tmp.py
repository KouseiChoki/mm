...
LastEditTime: 2026-04-09 09:15:28
Description: 
         ▄              ▄
        ▌▒█           ▄▀▒▌     
        ▌▒▒▀▄       ▄▀▒▒▒▐
...
def exr_read_worldpos(filePath):
    ...
    data = {}
    camera_type = 1
    data['focal_length'] = float(15.0)
    data['sensor_w'] = float(23.76)
    data['fov'] = float(76.758438)
    ...
    if mode == 1:
        ...
        for id,file_name in enumerate(file_names):
            ...
            if args.extra_depth is not None:
                extra_depth_path = os.path.join(args.extra_depth)
                extra_depth = jhelp_file(extra_depth_path)
