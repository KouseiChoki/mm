...
LastEditTime: 2023-07-03 11:18:00
...
def exr_imread(filePath):
    ...
    rm = np.array(array.array('f', img_exr.channel('FinalImagePWMask.R',pt))).reshape(size)
    gm = np.array(array.array('f', img_exr.channel('FinalImagePWMask.G',pt))).reshape(size)
    bm = np.array(array.array('f', img_exr.channel('FinalImagePWMask.B',pt))).reshape(size)
    # am = np.array(array.array('f', am)).reshape(size)
    mask = np.stack([rm,gm,bm],axis=2).clip(0,1)*255
    return image.astype('uint8'),mask.astype('uint8')
...
if __name__=='__main__':
    ...
    for i in tqdm(range(len(sources))):
        ...
        cv2.imwrite(os.path.join(mask_save_path,name)+'.png',mask[...,::-1])
