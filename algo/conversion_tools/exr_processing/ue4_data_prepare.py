...
def subpro(args):
    ...
    # bg_save_path = os.path.join(save_path,'bg_masked')
    # bg_mv1_masked_save_path = os.path.join(save_path,'bg_mv1_masked')
    name = os.path.basename(source).replace('.exr','')
    if not os.path.isfile(os.path.join(mask_save_path,name)+'.png'):
        ...
        write_flo_file(mv1, os.path.join(mv1_save_path,name)+'.flo')
        cv2.imwrite(os.path.join(mask_save_path,name)+'.png',np.stack([mask,mask,mask],axis=2))
        
        # image_ = image.copy()
        # mv1_ = mv1.copy()
        # image[np.where(mask==0)] = 0
        # mv1[np.where(mask==0)] = 0
        # cv2.imwrite(os.path.join(maskimg_save_path,name)+'.png',image)
        # write_flo_file(mv1, os.path.join(mv1_masked_save_path,name)+'.flo')

        # image_[np.where(mask!=0)] = 0
        # mv1_[np.where(mask!=0)] = 0
        # cv2.imwrite(os.path.join(bg_save_path,name)+'.png',image_)
        # write_flo_file(mv1_, os.path.join(bg_mv1_masked_save_path,name)+'.flo')

if __name__=='__main__':
    assert len(sys.argv)==3 ,'usage: python exr_2_zipexr.py root'
    ...
    mkdir(bg_save_path)
    mkdir(bg_mv1_masked_save_path)
    data = [(sources[i],save_path) for i in range(len(sources))]
    process_map(subpro, data, max_workers=8)
