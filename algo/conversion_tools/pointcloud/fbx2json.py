...
def fbx_reader(fn,sp=None):
    import re
    res = []
    print("reading from: %r " % fn, end="")
    fbx_root_elem, fbx_version = parse(fn, use_namedtuple=True)
    print("(Version %d) ..." % fbx_version)
    ident_sub = "    "
    for fbx_elem_sub in fbx_root_elem.elems:
        fbx2json_recurse(res.append, fbx_elem_sub, ident_sub,
                            fbx_elem_sub is fbx_root_elem.elems[-1])
    if sp is not None:
        with open(sp, 'w', encoding="ascii", errors='xmlcharrefreplace') as f:
            fw = f.write
            fw('[\n')
            ident_sub = "    "
            for fbx_elem_sub in fbx_root_elem.elems:
                fbx2json_recurse(f.write, fbx_elem_sub, ident_sub,
                                fbx_elem_sub is fbx_root_elem.elems[-1])
            fw(']\n')
    # 正则表达式模式：匹配 "KeyValueFloat" 和后面的数组
    pattern = r'"KeyValueFloat",\s*\[\[([-+eE0-9.,\s]+)\]\]'
    # 使用 re.finditer 查找所有匹配
    data = ''.join(res)
    ...
    for match in matches:
        array_str = match.group(1)  # 获取数组部分的字符串
...
def main():
    ...
    if "--help" in sys.argv:
        print(__doc__)
        return
    # fn = '/Users/qhong/Downloads/UE_pattern_1113.fbx'
