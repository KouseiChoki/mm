 ...
 * @LastEditTime: 2024-04-17 15:46:21
...
void MCRender(FrameData *pTriDataIn_, MVD *pTriMVFI_, Point2D *pRenderTriVeterx_, DTYPE *pTriDepthFI_, DTYPE x, DTYPE y, DTYPE *pZBuffer, FrameData *pDataOut, int offset, int EdgeFlag )
{
...
void FI3D_Core(MVD *pMVIn, FrameData *pDataIn, DTYPE *pZBufferIn, DTYPE nPhaseIn ,FrameData *pFIBuffer)
{
...
//                __FLT_MIN__;
            ...
            if ((isFIEnable_u || (nAddEdge_u>0 && isFIEnable_u==0)) && has_fb==0) {
                MCRender(pInTriData, mv_fi,pOutTriVertex, nDepthFI, x, y, pZBufferIn, pFIBuffer,0,0);
            }
            else
            {
                if (pstParms->m_nFillHoleWithFG) {
                    ...
                    MCRender(pInTriData, mv_fi,pOutTriVertex, nDepthFI, x, y, pZBufferIn, pFIBuffer,0,1);
                }
            }
            ...
            if ((isFIEnable_l || (nAddEdge_l>0&& isFIEnable_l == 0)) && has_fb==0) {
                MCRender(pInTriData, mv_fi, pOutTriVertex, nDepthFI, x, y, pZBufferIn, pFIBuffer,1,0);
            ...
            }else
            {
                if (pstParms->m_nFillHoleWithFG) {
                    ...
                    MCRender(pInTriData, mv_fi,pOutTriVertex, nDepthFI, x, y, pZBufferIn, pFIBuffer,1,1);
                }
            }
        }
...
void updatePrams(DTYPE EdgeThr) {
    pstParms->m_nMVEdgeThr = EdgeThr;
}

extern "C"
void fn(float* image,float* mv,int h,int w,DTYPE nPhase,DTYPE m_nMVEdgeThr)

{
    ...
    FI3D_Core(pMVIn, pDataIn, pDepthO, nPhase, pRPOut);
