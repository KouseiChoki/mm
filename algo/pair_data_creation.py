...
LastEditTime: 2025-05-15 13:30:11
Description: 
         ▄              ▄
        ▌▒█           ▄▀▒▌     
        ▌▒▒▀▄       ▄▀▒▒▒▐
    ...
    try:
        ...
        if args.prune_repeat_frame:
            for i in range(1,len(img_stack)-1):
                if repeat_frame_check(img_stack[i-1],img_stack[i]): #cf mv1
                    offset = args.num_frames - 2
                    index = i - 1 + offset
                    if 0 <= index < len(valid):
                        valid[index] = False
                    else:
                        print(f"[Warning] Skipping invalid index: {index}")
                    # valid[i - 1 + (args.num_frames-2)] = False
                if repeat_frame_check(img_stack[i],img_stack[i+1]): #cf mv0
                    offset = 0
                    index = i - 1 + offset
                    if 0 <= index < len(valid):
                        valid[index] = False
