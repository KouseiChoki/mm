...
class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, oneside=False, input_frames=5, reverse_rate=0.3,max_mv=1000):
        self.augmentor = None
        self.sparse = sparse
    ...
    def __getitem__(self, index):
        ...
        if valids is None:
            valids = [((flow[0].abs() < self.max_mv) & (flow[1].abs() < self.max_mv)).float() for flow in flows]
        else:
...
class MpiSintelTrain(FlowDatasetTest):
    def __init__(self, data_base = '',return_gt=True, input_frames=5, dstype='clean'):
        super(MpiSintelTrain, self).__init__(input_frames=input_frames, return_gt=return_gt)
        root = osp.join(data_base,'Sintel/')
        self.image_list = []
        self.flow_list = []
        self.extra_info_list = []
...
class FlyingThings3D(FlowDataset):
    def __init__(self,data_base='', aug_params=None, input_frames=5, dstype='frames_cleanpass'):
        super(FlyingThings3D, self).__init__(aug_params=aug_params, input_frames=input_frames, oneside=False, sparse=False)
        root = osp.join(data_base,'flyingthings/')
        self.image_list = []
        self.flow_list = []
        self.has_gt_list = []
