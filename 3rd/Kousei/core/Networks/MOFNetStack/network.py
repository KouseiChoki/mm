...
from ..encoders import twins_svt_large, convnext_Xlarge_4x, convnext_base_2x,BasicEncoder,BasicEncoder2,BasicEncoder3
...
class MOFNet(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.cnet == 'twins':
            ...
            self.cnet = twins_svt_large(pretrained=self.cfg.pretrain,mps=platform.machine().lower() =='arm64')
        elif cfg.cnet == 'basicencoder':
            # print("[Using basicencoder as context encoder]")
            self.cnet = BasicEncoder(output_dim=256, norm_fn='instance')
        elif cfg.cnet == 'basicencoder2':
            # print("[Using basicencoder as context encoder]")
            self.cnet = BasicEncoder2(output_dim=256, norm_fn='instance')
        elif cfg.cnet == 'basicencoder3':
            # print("[Using basicencoder as context encoder]")
            self.cnet = BasicEncoder3(output_dim=256, norm_fn='instance')
        ...
        if cfg.fnet == 'twins':
            ...
            self.fnet = twins_svt_large(pretrained=self.cfg.pretrain,mps=platform.machine().lower() =='arm64')
        ...
        elif cfg.fnet == 'basicencoder2':
            # print("[Using basicencoder as context encoder]")
            self.fnet = BasicEncoder2(output_dim=256, norm_fn='instance')
        elif cfg.fnet == 'basicencoder3':
            # print("[Using basicencoder as context encoder]")
            self.fnet = BasicEncoder3(output_dim=256, norm_fn='instance')
        ...
        self.mask = nn.Sequential(
            ...
            nn.Conv2d(256//hidden_dim_ratio, cfg.down_ratio**2*9*2, 1, padding=0))
    ...
    def upsample_flow_kousei(self, flow, mask,scale=8):
    ...
    def forward(self, images, data={}, flow_init=None):
        ...
        for itr in range(self.cfg.decoder_depth):
            ...
            with autocast(enabled=self.cfg.mixed_precision):
                net, motion_hidden_state, delta_flow = self.update_block(net, motion_hidden_state, inp, forward_corr, backward_corr, forward_flow, backward_flow, forward_coords0, attention, bs=B)
            up_mask = 100.0 * self.mask(net.float())
            ...
            if down_ratio == 4:
                ...
                forward_flow_up = self.upsample_flow_kousei(forward_coords1-forward_coords0, forward_up_mask,scale=down_ratio)
                backward_flow_up = self.upsample_flow_kousei(backward_coords1-backward_coords0, backward_up_mask,scale=down_ratio)
