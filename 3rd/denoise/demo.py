...
def load_img(filepath):
    ...
    return read(filepath,type='image')

def save_img(filepath, img):
    if '.exr' not in filepath:
        cv2.imwrite(filepath,cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
    else:
        write(filepath,img.astype('float32')/255)
...
# extensions = ['jpg', 'JPG', 'png', 'PNG', 'jpeg', 'JPEG', 'bmp', 'BMP','exr']
...
try:
    ...
    sys.exit('[MM ERROR],YOU DONT HAVE MODEL FILE!,请下载模型文件至/mm/3rd/denoise/Denoising/pretrained_models目录下')
...
with torch.no_grad():
    for file_ in tqdm(files):
        ...
        output = os.path.join(out_dir,os.path.relpath(file_, args.input_dir))

        if task == 'Gaussian_Gray_Denoising':
            save_gray_img(output, restored)
        else:
            save_img(output, restored)
