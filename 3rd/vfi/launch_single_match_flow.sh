...
VFI_CONFIG="train_config_0825_single_match_flow.yaml"
...
if [[ -f "${VFI_PID_FILE}" ]]; then
  ...
  if [[ -n "${VFI_OLD_PID}" ]] && kill -0 "${VFI_OLD_PID}" 2>/dev/null; then
    echo "single-match训练已经在运行，PID=${VFI_OLD_PID}"
    exit 1
  fi
fi

export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True"
echo "执行single-match完整训练图显存preflight（两个phase尺寸）"
"${VFI_PYTHON}" smoke_single_match.py \
  --config "${VFI_CONFIG}" \
...
echo "single-match长训练已启动，PID=${VFI_PID}"
echo "日志: ${VFI_LOG}"
