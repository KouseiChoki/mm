...
def decode_video_ffmpeg_pipe(
    ...
    if resize:
        ...
        # 不resize时，用探测到的原始分辨率作为输出尺寸
        probed_w, probed_h, _ = _probe_video(video_path)
        if probed_w == 0 or probed_h == 0:
            logger.warning(f"    ffprobe 无法获取分辨率，回退使用目标尺寸 {width}x{height}")
            out_width, out_height = width, height
        else:
            out_width, out_height = probed_w, probed_h

    cmd = ["ffmpeg", "-v", "error", "-i", str(video_path), "-f", "rawvideo", "-pix_fmt", "bgr24"]
    if resize:
        cmd += ["-vf", f"scale={out_width}:{out_height}:flags=lanczos"]
    cmd += ["pipe:1"]
    ...
    while True:
        ...
        if len(raw) < frame_bytes:
            break
        frame = np.frombuffer(raw, dtype=np.uint8).reshape((out_height, out_width, 3))
...
def stage_decode(
    ...
    for video_path in tqdm(videos, desc="解码", unit="video"):
        ...
        try:
            if use_ffmpeg:
                n = decode_video_ffmpeg_pipe(video_path, out_dir, resize, width, height, fmt, jpg_quality)
            else:
                n = decode_video_opencv_pipe(video_path, out_dir, resize, width, height, fmt, jpg_quality)
