...
class FlowDatasetKousei(data.Dataset):
    def __init__(self, data_root,aug_params=None, sparse=False, input_frames=2,norm=False,reverse=False,OPENEXR=True):
        ...
        if aug_params['mask_type'] == 'mix':
            ...
            self.mask_type = aug_params['mask_type'] if not aug_params['mask_type'] or aug_params['mask_type'].lower()!='frame' else None
    ...
    def __getitem__(self, index):
        ...
        if self.mask_list[index] is None:
            masks = [None]*len(imgs)
        else:
        ...
        # film grain
        grain_strength = float(self.film_grain[0])
        grain_prob = float(self.film_grain[1])

        if grain_strength > 0 and np.random.rand() < grain_prob:
            scale = 1
            imgs = [
                (
                    image
                    + scale
                    * np.random.uniform(-grain_strength, grain_strength)
