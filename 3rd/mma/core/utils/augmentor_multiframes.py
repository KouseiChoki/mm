...
class FlowAugmentor:
    def __init__(self, aug_params):
        ...
        self.do_flip = aug_params['do_flip']
    ...
    def aug_helper1(self,img):
        return np.array(self.photo_aug(Image.fromarray(img)), dtype=np.uint8)
    def aug_helper2(self,img):
        img = torch.FloatTensor(np.transpose(img,(2,0,1)))
        return np.transpose(self.photo_aug(img).numpy(),(1,2,0))
    def aug_helper(self,img):
        mask = None
        if img.shape[2]==4 :#mask case:
            mask= np.copy(img[...,-1])
            img = img[...,:3]
        if self.img_type == 'image':
            result = self.aug_helper1(img)
        else:
            result = self.aug_helper2(img)
        if mask is not None:
    ...
    def color_transform(self, imgs):
        ...
        if np.random.rand() < self.asymmetric_color_aug_prob:
            imgs = [self.aug_helper(img) for img in imgs]

        # symmetric
        else:
            img_num = len(imgs)
            image_stack = np.concatenate(imgs, axis=0)
            image_stack = self.aug_helper(image_stack)
    ...
    def spatial_transform(self, imgs, flows):
        ...
        ht, wd = imgs[0].shape[:2]
        if self.max_scale != 0 and self.min_scale!=0:
            ...
            if np.random.rand() < self.spatial_aug_prob:
                ...
                for i in range(len(flows)):
                    if flows[i] is not None:
                        if flows[i].shape[-1]>2: #valid interpolation
                            tmp = flows[i][...,:2]
                            tmp = cv2.resize(tmp, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_CUBIC)
                            tmp_valid = flows[i][...,-1]
                            tmp_valid = cv2.resize(tmp_valid, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
                            tmp = np.concatenate((tmp,tmp_valid[...,None]),axis=-1)
                            flows[i] = tmp
                        else:
                            flows[i] = cv2.resize(flows[i], None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_CUBIC)
                        flows[i][...,:2]  *= [scale_x, scale_y]
        ...
        if imgs[0].shape[0] == self.crop_size[0]:
            ...
            if imgs[0].shape[0] - self.crop_size[0] <=0 :
                print(imgs[0].shape[0],self.crop_size[0])
        ...
        flows = [flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]] if flow is not None else None for flow in flows]
    ...
    def __call__(self, imgs, flows):
        ...
        flows = [np.ascontiguousarray(flow) if flow is not None else None for flow in flows]
        
        return imgs, flows



class FlowAugmentor_Mask:
