...
# source ~/.zshrc
if grep -q "mmip()" ~/.zshrc; then
    echo "命令 'mmip()' 已存在于 ~/.zshrc 中，跳过添加。"
else
    # 如果命令不存在，则添加到 ~/.zshrc
    echo "添加命令 'mmip()' 到 ~/.zshrc。"
    echo 'mmip(){
        conda activate mm
        cd '"$PARENT_DIR"'
...
if grep -q "mmlama()" ~/.zshrc; then
    echo "命令 'mmlama()' 已存在于 ~/.zshrc 中，跳过添加。"
else
    # 如果命令不存在，则添加到 ~/.zshrc
    echo "添加命令 'mmlama()' 到 ~/.zshrc。"
    echo 'mmlama(){
        conda activate mm
        cd '"$PARENT_DIR"'
        cd 3rd/lama
        export PYTORCH_ENABLE_MPS_FALLBACK=1 && python -W ignore start.py "$@"
    }' >> ~/.zshrc
fi
